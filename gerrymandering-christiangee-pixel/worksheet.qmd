...
votes |>
mutate(vi = r_votes/(r_votes + d_votes))|>
summarize(mean = mean(vi),
    median = median(vi)) |>
mutate(mean-median)
...
votes_eg <- votes |>
    mutate(d_wasted +sapply(x=d_wasted, fun = wasted),
        r_wasted = sapply(x= r_votes, fun = wasted)) |>
    mutate(effiency_gap = (r_wasted - d_wasted)/100)
